...
class AbstractUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.final_conv(x)
        '''
        attention_weights = []  # Initialize an empty list to store attention weights
        for decoder, encoder_features in zip(self.decoders, encoders_features):
            # pass the output from the corresponding encoder and the output
            # of the previous decoder
            x = decoder(encoder_features, x)  # Assuming attn_weights is provided by decoder
            # x, attn_weights = decoder(encoder_features, x)  # Assuming attn_weights is provided by decoder
            # attention_weights.append(attn_weights)  # Append attention weights to the list
        ...
        return x
        # print('returning x and attention_weights')
        # return x, attn_weights

## the model we use !! 3D and basic_module=DoubleConv
